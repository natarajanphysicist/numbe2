...
def calculate_primary_fluence_numba(source, direction, density_grid, grid_shape):
    fluence = np.zeros(grid_shape, dtype=np.float32) # Use float32
...
class QRadPlan3D:
    def __init__(self, grid_size=(100, 100, 100), num_beams=8, kernel_path="dose_kernel.npy",
        ...
        self.dose_kernel = np.load(kernel_path).astype(np.float32) # Ensure float32
        ...
        self.num_phases = 10
        logger.info(f"QRadPlan3D __init__: Initial self.num_phases = {self.num_phases}")
        self.respiratory_phase_weights = np.ones(self.num_phases) / self.num_phases
        self.accumulated_dose = np.zeros(self.grid_size) # Initialize with default grid_size
        ...
        if fourd_ct_path and dicom_rt_struct_path:
            ...
            try:
                ...
                if self.density_grids_phases: # Grid size might change
                    self.grid_size = self.density_grids_phases[0].shape
                    logger.info(f"Grid size updated to {self.grid_size} based on loaded 4D CT data.")
                    self.accumulated_dose = np.zeros(self.grid_size) # Re-init with new grid size
                ...
                self._initialize_simplified_model(grid_size) # Pass original grid_size for fallback
        
        elif dicom_rt_struct_path and ct_path: # If static CT paths are provided
            logger.info(f"Attempting to load static 3D CT data from: {ct_path}")
            logger.info(f"Attempting to load RTStruct from: {dicom_rt_struct_path}")
            logger.info(f"QRadPlan3D __init__: Before static CT load, self.num_phases = {self.num_phases}")
            try:
                ...
                num_slices_loaded = len(ref_ct_series) # Use actual number of loaded slices
                self.grid_size = (cols, rows, num_slices_loaded) # Update grid_size based on loaded static CT
                logger.info(f"Grid size updated to {self.grid_size} from static CT.")
                self.accumulated_dose = np.zeros(self.grid_size) # Re-init with new grid size

                ct_pixel_data = np.stack([s.pixel_array * s.RescaleSlope + s.RescaleIntercept for s in ref_ct_series], axis=-1)
                ct_pixel_data = ct_pixel_data.transpose(1, 0, 2) # (cols, rows, slices)
                ...
                # For static CT, effectively 1 phase of data repeated
                self.num_phases = 1 # Explicitly set num_phases to 1 for static 3D data
                self.respiratory_phase_weights = np.ones(self.num_phases) / self.num_phases
                logger.info(f"Static 3D CT loaded. Setting num_phases = {self.num_phases}")


                self.density_grids_phases = [density_grid_ref] # Store as list with one element
                self.tumor_masks_phases = [tumor_mask_ref]   # Store as list with one element
                self.oar_masks_phases = [oar_masks_ref]     # Store as list with one element
                self.affine_transforms = [np.eye(4)]         # Store as list with one element
        ...
        else: # Neither 4D nor static 3D paths provided
            logger.info("No DICOM paths provided by DataManager. Initializing with simplified model using DataManager's grid_size.")
            # DataManager provides grid_size based on loaded DICOMs, but no paths to QRadPlan3D
            # So QRadPlan3D uses its simplified model but with the externally provided grid_size.
            self._initialize_simplified_model(grid_size) # grid_size here is from DataManager via QRadPlan3D call
            logger.info(f"QRadPlan3D __init__: After _initialize_simplified_model (in final else), self.num_phases = {self.num_phases}")
    ...
    # --- QRadPlannerApp/backend/radiotherapy_planner.py ---
# ... inside QRadPlan3D class ...
...
# ... inside QRadPlan3D class ...
    def _initialize_simplified_model(self, grid_size_param):
        ...
        # --- ENSURE THIS MODIFICATION IS SAVED ---
        self.num_phases = 1 # Reduce number of phases for simplified model to save memory
        # Also update the respiratory_phase_weights accordingly if num_phases changes
        self.respiratory_phase_weights = np.ones(self.num_phases) / self.num_phases
        logger.info(f"_initialize_simplified_model: self.num_phases explicitly SET TO: {self.num_phases}")
        # --- END MODIFICATION ---
        ...
        logger.info(f"Initialized with simplified model, grid_size: {self.grid_size}, num_phases: {self.num_phases}")
...
# ... inside QRadPlan3D class ...
    ...
    def set_tumor_data(self, tumor_center=None, tumor_size=None, tumor_mask_input=None):
        ...
        The planner expects the tumor mask to match its internal grid_size (cols, rows, slices) -> (x,y,z).
        The input tumor_mask_input from DataManager is typically (slices, rows, cols) -> (z,y,x).
        """
        try:
            if tumor_mask_input is not None:
                # Expected planner grid orientation: (x, y, z) -> (cols, rows, slices)
                # Input mask orientation: (z, y, x) -> (slices, rows, cols)
                
                # Check if a transpose is needed to match planner's (x,y,z) convention
                # tumor_mask_input.shape = (slices, rows, cols)
                # self.grid_size = (cols, rows, slices)
                
                # Transpose tumor_mask_input from (z,y,x) to (x,y,z)
                # (slices, rows, cols) -> (cols, rows, slices)
                # axes: (0,1,2) -> (2,1,0)
                if tumor_mask_input.ndim == 3:
                    transposed_mask = np.transpose(tumor_mask_input, (2, 1, 0)) # z,y,x -> x,y,z
                    logger.info(f"Transposing input tumor mask from {tumor_mask_input.shape} to {transposed_mask.shape} to match planner grid {self.grid_size}")
                else:
                    logger.error(f"tumor_mask_input is not 3D, shape: {tumor_mask_input.shape}")
                    # Fallback or raise error - for now, try to use as is if not 3D (though unlikely)
                    transposed_mask = tumor_mask_input


                if transposed_mask.shape != self.grid_size:
                    # If shapes still don't match after transpose (e.g., due to resampling elsewhere or different image source)
                    # then attempt resize. This should be less common if DataManager and Planner grid are derived consistently.
                    from scipy.ndimage import zoom
                    logger.warning(f"Resizing transposed tumor mask from {transposed_mask.shape} to {self.grid_size} (this might indicate an upstream inconsistency)")
                    
                    zoom_factors = tuple(gs_dim / tm_dim for gs_dim, tm_dim in zip(self.grid_size, transposed_mask.shape))
                    
                    # Perform zoom on a float version, then convert to bool
                    # Ensure input to zoom is float
                    tumor_mask_resized_float = zoom(transposed_mask.astype(float), zoom_factors, order=0)
                    resized_mask = tumor_mask_resized_float.astype(bool)

                    if resized_mask.shape != self.grid_size:
                         logger.warning(f"Resized mask shape {resized_mask.shape} still not matching grid {self.grid_size}. Cropping/padding.")
                         # Simple crop/pad to force match - this is crude
                         cropped_padded_mask = np.zeros(self.grid_size, dtype=bool)
                         min_shape = [min(s_g, s_r) for s_g, s_r in zip(self.grid_size, resized_mask.shape)]
                         slices_to_crop = tuple(slice(0, ms) for ms in min_shape)
                         
                         target_slices = tuple(slice(0, ms) for ms in min_shape) # Slices for the target array
                         cropped_padded_mask[target_slices] = resized_mask[slices_to_crop]
                         self.tumor_mask = cropped_padded_mask
                    else:
                         self.tumor_mask = resized_mask
                else:
                    self.tumor_mask = transposed_mask.astype(bool) # Ensure boolean
            
            elif tumor_center is not None and tumor_size is not None:
                # This part creates mask directly in planner's grid orientation (x,y,z)
                ...
                if self.tumor_mask is None: # Check if tumor_mask was set by DICOM loading
            ...
            # This part assumes self.tumor_mask is now in the planner's (x,y,z) orientation.
            # However, self.tumor_masks_phases is expected by other parts of the planner
            # (like simulate_fractionated_treatment) to be in (z,y,x) if it interacts with self.calculate_dose
            # which uses density_grids_phases (z,y,x). This needs careful review.

            # For now, let's assume tumor_masks_phases should also be in (x,y,z) consistent with self.tumor_mask here.
            # If not, it needs to be transposed back before being used with density grids.
            # The original code for self.tumor_masks_phases was:
            # self.tumor_masks_phases = [self.tumor_mask.copy() for _ in range(self.num_phases)]
            # This will copy the (x,y,z) oriented mask.

            if not hasattr(self, 'tumor_masks_phases') or self.tumor_masks_phases is None or len(self.tumor_masks_phases) != self.num_phases:
                # If tumor_masks_phases needs to be (z,y,x) then we'd transpose self.tumor_mask back here.
                # For now, keeping it consistent with self.tumor_mask's (x,y,z) orientation after the above logic.
                self.tumor_masks_phases = [self.tumor_mask.copy() for _ in range(self.num_phases)]
            else:
                for i in range(self.num_phases):
                    self.tumor_masks_phases[i] = self.tumor_mask.copy()
            
            # Update ITV (union of tumor masks across phases) - this assumes tumor_masks_phases are all (x,y,z)
            if self.tumor_masks_phases:
                # If self.tumor_mask (ITV) is used by objective function directly, it should be (x,y,z)
                # self.tumor_mask here refers to the ITV.
                # The self.tumor_mask which is member of QRadPlan3D used in optimization objective
                # should be in (x,y,z) to match dose_influence_phases[phase_idx, i_beam, :, :, :]
                self.tumor_mask = np.any(self.tumor_masks_phases, axis=0) # This creates the ITV for planner.
            
            logger.info(f"Tumor data set. Planner ITV shape: {self.tumor_mask.shape}, Volume: {np.sum(self.tumor_mask)} voxels.")
            return True
    ...
    def _initialize_simplified_model(self, grid_size_param):
        self.grid_size = grid_size_param 
        self.num_phases = 1 # Reduce number of phases
        self.respiratory_phase_weights = np.ones(self.num_phases) / self.num_phases
        self.tumor_center = np.array(self.grid_size) / 2
        self.tumor_mask = self._create_spherical_mask(self.tumor_center, 10)
        self.tumor_mask_name = "Simulated Tumor"
        ...
        self.accumulated_dose = np.zeros(self.grid_size) 
        self.voxel_volume = 1.0 * 1.0 * 1.0 * 1e-3 
        logger.info(f"Initialized with simplified model, grid_size: {self.grid_size}, self.num_phases SET TO: {self.num_phases}") # Explicitly log self.num_phases
    ...
    def optimize_beams(self):
       # logger.info("  [optimize_beams] Starting classical optimization...")
        logger.info(f"  [optimize_beams] Starting classical optimization. Current self.num_phases = {self.num_phases}") # DEBUG LOG
