...
class QRadPlan3D:
    def __init__(self, grid_size=(100, 100, 100), num_beams=8, kernel_path="dose_kernel.npy",
        ...
        self.num_phases = 10 
        logger.info(f"QRadPlan3D __init__: Initial self.num_phases = {self.num_phases}") # DEBUG LOG
        ...
        if fourd_ct_path and dicom_rt_struct_path:
            ...
            logger.info(f"Attempting to load RTStruct from: {dicom_rt_struct_path}")
            logger.info(f"QRadPlan3D __init__: After static CT load, self.num_phases = {self.num_phases}") # DEBUG LOG
            
        else:
            logger.info("No DICOM paths provided. Initializing with simplified model.")
            self._initialize_simplified_model(grid_size)
            logger.info(f"QRadPlan3D __init__: After _initialize_simplified_model, self.num_phases = {self.num_phases}") # DEBUG LOG
            try:
